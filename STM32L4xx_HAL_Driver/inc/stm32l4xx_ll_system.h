...
// Flash errors for STM32L4xx
#define FLASH_ECCR_ERRORS        (FLASH_ECCR_ECCD | FLASH_ECCR_ECCC)
...
#define FLASH_SR_ERRORS      (FLASH_SR_OPERR   | FLASH_SR_PROGERR | FLASH_SR_WRPERR | \
                                   ...
                                   FLASH_SR_OPTVERR | FLASH_SR_PEMPTY)
#else
#define FLASH_SR_ERRORS      (FLASH_SR_OPERR   | FLASH_SR_PROGERR | FLASH_SR_WRPERR | \
                                   FLASH_SR_PGAERR  | FLASH_SR_SIZERR  | FLASH_SR_PGSERR | \
                                   FLASH_SR_MISERR  | FLASH_SR_FASTERR | FLASH_SR_RDERR  | \
...
#define FLASH_ALL_ERRORS         (FLASH_SR_ERRORS | FLASH_ECCR_ERRORS)

#define FLASH_GET_FLAG(__FLAG__)          ((((__FLAG__) & FLASH_ECCR_ERRORS) != 0U)     ? \
                                                 (READ_BIT(FLASH->ECCR, (__FLAG__)) != 0U) : \
                                                 (READ_BIT(FLASH->SR,   (__FLAG__)) != 0U))

#define FLASH_CLEAR_FLAG(__FLAG__)        do { if(((__FLAG__) & FLASH_ECCR_ERRORS) != 0U) { SET_BIT(FLASH->ECCR, ((__FLAG__) & FLASH_ECCR_ERRORS)); }\
                                                     if(((__FLAG__) & ~(FLASH_ECCR_ERRORS)) != 0U) { WRITE_REG(FLASH->SR, ((__FLAG__) & ~(FLASH_ECCR_ERRORS))); }\
                                                   } while(0)
...
//
// Cortex NVIC macros - BG5HHP 2022.05
//
...
#define NVIC_ENABLE_IRQ(irqn, p0, p1)         \
{                                             \
    NVIC_SetPriority(irqn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(), p0, p1));    \
    NVIC_EnableIRQ(irqn);                     \
}
